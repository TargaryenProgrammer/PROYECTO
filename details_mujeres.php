<?php
/**
 * Script para mostrar los detalles de los mujeres
...
if ($id == '' || $token == '') {
    ...
    if ($token == $token_tmp) {
        $sql = $con->prepare("SELECT count(id) FROM mujeres WHERE id = ? AND activo = 1");
        $sql->execute([$id]);
        if ($sql->fetchColumn() > 0) {
            $sql = $con->prepare("SELECT id, nombre, descripcion, precio, descuento FROM mujeres WHERE id = ? AND activo = 1 LIMIT 1");
            $sql->execute([$id]);
            ...
            $sqlCaracter = $con->prepare("SELECT DISTINCT(det.id_caracteristica) AS idCat, cat.caracteristica FROM det_muj_caracter AS det INNER JOIN caracteristicas AS cat ON det.id_caracteristica=cat.id WHERE det.id_mujer = ?");
                    ...
                    <div class="col-md-7 order-md-2">
                        ...
                        <div class="col-3 my-3">
                            ...
                            while ($row_cat = $sqlCaracter->fetch(PDO::FETCH_ASSOC)) {
                                ...
                                $sqlDet = $con->prepare('SELECT id, valor, stock FROM det_muj_caracter WHERE id_mujer=? AND id_caracteristica=?');
                        ...
                        <div class="d-grid gap-3 col-7">
                            ...
                            <button class="btn btn-secondary btn-outline" type="button"
                                onclick="addProdMujer(<?php echo $row['id']; ?>, '<?php echo hash_hmac('sha1', $row['id'], KEY_TOKEN); ?>')">Agregar
                                al carrito</button>
                        </div>
                    </div>
                </div>
