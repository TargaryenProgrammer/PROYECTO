...
$sql = "UPDATE mujeres SET nombre = ?, descripcion = ?, precio = ?, descuento = ?, stock = ?, id_categoria = ? WHERE id = ?";
...
if ($stm->execute([$nombre, $descripcion, $precio, $descuento, $stock, $categoria, $id])) {
    ...
    if ($sizeTalla == count($color) && $sizeTalla == count($precioVariante) && $sizeTalla == count($stockVariante)) {
        $sqlVariante = "INSERT INTO mujeres_variantes(id_mujer, id_talla, id_color, precio, stock) VALUES(?,?,?,?,?)";
        $stm = $con->prepare($sql);
